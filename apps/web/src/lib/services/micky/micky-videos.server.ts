...
import { sponsorToVideos, sponsors, videos } from '@r8y/micky-data/schema'
import { mickyDb as defaultDb } from '@/db/micky.client.server'
import { VIDEO_PAGE_SIZE, type VideoSummary } from './micky.types'
import {
  asIsoDate,
  asNumber,
  normalizePage,
...
export namespace MickyVideosService {
      ...
      try: () => loadVideos(idsAndTotalResult.value.videoIds),
        ...
        new VideosQueryError({
          message:
            cause instanceof Error
              ? cause.message
              : 'Failed to hydrate Micky videos',
        }),
    })
