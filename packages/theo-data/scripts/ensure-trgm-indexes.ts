...
await sql`
  CREATE INDEX CONCURRENTLY IF NOT EXISTS theo_videos_title_trgm
    ON theo.videos
    USING gin (title gin_trgm_ops)
`;

await sql`
  CREATE INDEX CONCURRENTLY IF NOT EXISTS theo_sponsors_name_trgm
    ON theo.sponsors
    USING gin (name gin_trgm_ops)
`;

console.log("theo: pg_trgm GIN indexes ensured");
