<script lang="ts">
  ...
  const submitXPost = async (xPostUrl: string) => {
    xPostSaveError = null
    xPostSaveError = await saveXPost(xPostUrl)
  }
</script>
      ...
      <div class="space-y-4">
        ...
        >
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            >
              {xPostPending > 0 ? 'Saving...' : 'Save'}
            </button>
            {#if video.xPost?.url}
              <button
                type="button"
                disabled={xPostPending > 0}
                onclick={() => submitXPost(video.xPost?.url ?? '')}
              ...
              >
                {xPostPending > 0 ? 'Refreshing...' : 'Refresh data'}
              </button>
            {/if}
          </div>
          {#if xPostSaveError}
