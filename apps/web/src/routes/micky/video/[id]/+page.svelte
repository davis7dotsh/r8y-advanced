<script lang="ts">
  ...
  import {
    ...
    linkMickyVideoToXPost,
  } from '@/remote/micky.remote'
  import { toHref } from '@/utils/url'

  const search = $derived(
    parseMickyVideoSearch(Object.fromEntries(page.url.searchParams)),
  )
...
<ChannelVideoDetailContent
  ...
  detailHref={(params) =>
    toHref(`/micky/video/${encodeURIComponent(id)}`, params)}
