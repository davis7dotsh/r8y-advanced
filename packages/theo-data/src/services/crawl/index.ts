...
export namespace CrawlService {
  ...
  const refreshXMetrics = async (
    ...
    const metricResult = await fetchXPostMetrics({
    ...
    const metrics = {
      ...
      xViews: metricResult.value.xViews,
      xLikes: metricResult.value.xLikes,
      xReposts: metricResult.value.xReposts,
      xComments: metricResult.value.xComments,
    };
  ...
  export const crawlRss = async (
    ...
    const feedResult = await fetchRssVideoIds({
      channelId,
      externalError: (message) =>
        new CrawlExternalError({
          message,
    ...
    const videoIds = feedResult.value.videoIds;
