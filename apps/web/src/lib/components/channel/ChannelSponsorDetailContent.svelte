...
<svelte:boundary>
  ...
  {:else}
    ...
    <section class="space-y-6">
      ...
      >
        <p
          class="text-xs font-semibold uppercase tracking-[0.15em] text-violet-600"
        >
          {sponsorLabel}
        </p>
        <h2
          class="mt-1.5 text-2xl font-bold tracking-tight text-neutral-900 dark:text-neutral-100"
        >
          {sponsor.name}
        </h2>
        ...
        <dl class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <div
            class="rounded-lg border border-red-100 bg-red-50 p-3.5 dark:border-neutral-700 dark:bg-neutral-800"
          >
            <dt
              class="text-xs font-medium uppercase tracking-wide text-red-400 dark:text-red-400"
            >
              Total YT views
            </dt>
            <dd
              class="mt-1 text-lg font-semibold text-red-700 dark:text-red-400"
            >
          ...
          <div
            class="rounded-lg border border-sky-100 bg-sky-50 p-3.5 dark:border-neutral-700 dark:bg-neutral-800"
          >
            <dt
              class="text-xs font-medium uppercase tracking-wide text-sky-500"
            >
              Total X views
            </dt>
            <dd
              class="mt-1 text-lg font-semibold text-sky-700 dark:text-sky-400"
            >
              {stats.totalXViews > 0
          ...
          >
            ...
            >
              {daysSince(stats.lastPublishedAt)}
            </dd>
          </div>
          <div
            class="rounded-lg border border-neutral-100 bg-neutral-50 p-3.5 dark:border-neutral-700 dark:bg-neutral-800"
          >
            ...
            >
              {#if stats.bestPerformingVideo}
                {stats.bestPerformingVideo.viewCount.toLocaleString()} &mdash; {stats
                  .bestPerformingVideo.title}
              {:else}
                N/A
              {/if}
            </dd>
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <p class="text-sm text-neutral-500">
          ...
          <span class="font-medium text-neutral-900 dark:text-neutral-100"
            >{videos.items.length}</span
          >
          of
          <span class="font-medium text-neutral-900 dark:text-neutral-100"
            >{videos.pagination.total}</span
          >
          sponsor videos
        </p>
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
        {#each videos.items as video (video.videoId)}
          <VideoCard
            title={video.title}
      ...
      {#if mentions && mentionsPageHref}
        ...
        >
          <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p class="mt-0.5 text-xs text-neutral-400">
                {mentions.pagination.total.toLocaleString()} comments mentioning this
                sponsor, sorted by most liked
              </p>
            </div>
          ...
          {:else}
            <div class="space-y-2">
              {#each mentions.items as comment (comment.commentId)}
                <article
                  class="rounded-lg border border-neutral-100 bg-neutral-50 p-3.5 dark:border-neutral-700 dark:bg-neutral-800"
                >
                  <div
                  ...
                  >
                    ...
                    >
                    <a
                      href={videoHref(comment.videoId)}
