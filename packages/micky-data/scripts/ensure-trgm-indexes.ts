...
await sql`
  CREATE INDEX CONCURRENTLY IF NOT EXISTS micky_videos_title_trgm
    ON micky.videos
    USING gin (title gin_trgm_ops)
`;

await sql`
  CREATE INDEX CONCURRENTLY IF NOT EXISTS micky_sponsors_name_trgm
    ON micky.sponsors
    USING gin (name gin_trgm_ops)
`;

console.log("micky: pg_trgm GIN indexes ensured");
