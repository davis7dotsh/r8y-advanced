...
<svelte:boundary>
  ...
  {#snippet failed(error, _reset)}
    <ErrorState
      title="Unable to load Micky videos"
      message={error instanceof Error ? error.message : 'Unknown error'}
    />
  {/snippet}

  {@const payload = await getMickyVideos(search)}
  {#if payload.status === 'error'}
    <ErrorState title="Unable to load Micky videos" message={payload.error.message} />
  {:else}
    ...
    <section class="space-y-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <PaginationControls
          ...
          getHref={(p) => toHref('/micky', { page: p, q: search.q })}
        />
      </div>
