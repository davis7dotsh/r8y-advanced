  ...
  <section class="space-y-6">
      ...
      <div class="space-y-4">
        <div>
          <div class="flex items-start justify-between gap-3">
            ...
            <a
              href={`/share/davis/${encodeURIComponent(video.videoId)}`}
              target="_blank"
              rel="noopener noreferrer"
              title="Share"
        ...
        >
          <label for="davis-x-post-url" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300">
            Link X Post
          </label>
          <div class="flex flex-wrap items-center gap-2">
            ...
            <button
              type="submit"
              disabled={linkDavisVideoToXPost.pending > 0}
            ...
            >
              {linkDavisVideoToXPost.pending > 0 ? 'Saving...' : 'Save'}
            </button>
            {#if video.xPost?.url}
              <button
                type="button"
              ...
              >
                {linkDavisVideoToXPost.pending > 0 ? 'Refreshing...' : 'Refresh data'}
              </button>
            {/if}
          </div>
          {#if xPostSaveError}
    ...
    <section class="rounded-xl border border-neutral-200 bg-white p-5 dark:border-neutral-700 dark:bg-neutral-900">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-1 rounded-lg border border-neutral-200 p-0.5 dark:border-neutral-700">
            {#each SORT_OPTIONS as option}
              <a
                href={toHref(`/davis/video/${encodeURIComponent(id)}`, {
                  commentsPage: 1,
      ...
      <PaginationControls
        ...
        getHref={(nextPage) =>
          toHref(`/davis/video/${encodeURIComponent(id)}`, {
